...
class Database:
    ...
    def put_scored_entry(self,dataset_nr: int,team_nr: int,run_count_team: int,
    ...
    def put_confusion_matrix(self,run_id: int,confusion_matrix: List[int]) -> int:
        if confusion_matrix != None:
            cursor = self.mydb.cursor()
            content = [run_id]
            content.extend(confusion_matrix)
            content = tuple(content)
            sql = "INSERT INTO wki_confusion_tables(`scored_run_id`,`TP`,`FN`,`FP`,`TN`) \
            VALUES(%s,%s,%s,%s,%s)"
    ...
    def put_model(self,team_nr: int,model_name: str,parameter_dict: Dict[str,any]) -> int:
        ...
        sql = "INSERT INTO wki_models(team_nr,model_name,parameters) \
            VALUES(%s,%s,%s,%s)"
        val = (team_nr,model_name,parameters)
        cursor.execute(sql,val)
